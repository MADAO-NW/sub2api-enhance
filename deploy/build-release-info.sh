...
mkdir -p dist
(cd backend && CGO_ENABLED=0 GOTOOLCHAIN=go1.27.0 go run -ldflags "-X main.Version=$release_version -X main.Commit=$release_commit -X main.Date=$release_date -X main.BuildType=release" ./cmd/server --release-info) > dist/release.json
