...
func Router(c *config.Config, handler *audit.AdminHandler, proxy *ingress.Proxy, client *sub2api.Client, assets fs.FS, quota *quotafollow.Handler, updates *systemupdate.Handler) (*gin.Engine, error) {
	...
	a.POST("/batches/preview", handler.PreviewBatch)
	a.POST("/batches", handler.CreateBatch)
	a.GET("/batches", handler.ListBatches)
	a.GET("/batches/:id", handler.GetBatch)
	a.GET("/batches/:id/items", handler.GetBatchItems)
