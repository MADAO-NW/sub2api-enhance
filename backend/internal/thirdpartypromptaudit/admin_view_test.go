...
func TestEventListExplainsLatestOutcomeWithItsOwnJobRevision(t *testing.T) {
	...
	require.NoError(t, err)
	defer db.Close()
	mock.ExpectQuery(`SELECT COUNT\(\*\).*JOIN sub2api_enhance.third_party_prompt_audit_jobs decision_job ON decision_job.id=o.job_id`).WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(1))
	now := time.Now()
	...
	mock.ExpectQuery(`SELECT e.id.*decision_job.config_snapshot::json->'review_threshold'.*decision_job.id=o.job_id`).WithArgs(20, 0).WillReturnRows(rows)
