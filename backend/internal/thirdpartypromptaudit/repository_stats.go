...
func (r *Repository) Stats(ctx context.Context, q StatsQuery) (*Stats, error) {
	...
	result := &Stats{StatsQuery: q, Calls: []CallStats{}}
	...
	queries := []struct {
		...
		{`SELECT COALESCE(json_object_agg(decision,n),'{}') FROM (SELECT o.decision,count(*) n FROM sub2api_enhance.third_party_prompt_audit_outcomes o JOIN sub2api_enhance.third_party_prompt_audit_jobs j ON j.id=o.job_id WHERE j.run_kind='request' AND o.created_at>=$1 AND o.created_at<$2 AND ($3='' OR j.execution_mode=$3) GROUP BY o.decision UNION ALL SELECT 'partial_failure',count(*) FROM sub2api_enhance.third_party_prompt_audit_outcomes o JOIN sub2api_enhance.third_party_prompt_audit_jobs j ON j.id=o.job_id WHERE j.run_kind='request' AND o.partial_failure AND o.created_at>=$1 AND o.created_at<$2 AND ($3='' OR j.execution_mode=$3)) x`, args, &result.Formal},
		{`SELECT COALESCE(json_object_agg(decision,n),'{}') FROM (SELECT o.decision,count(*) n FROM sub2api_enhance.third_party_prompt_audit_outcomes o JOIN sub2api_enhance.third_party_prompt_audit_jobs j ON j.id=o.job_id WHERE j.run_kind='reaudit' AND o.created_at>=$1 AND o.created_at<$2 AND ($3='' OR j.execution_mode=$3) GROUP BY o.decision UNION ALL SELECT 'changed',count(*) FROM sub2api_enhance.third_party_prompt_audit_outcomes o JOIN sub2api_enhance.third_party_prompt_audit_jobs j ON j.id=o.job_id JOIN LATERAL (SELECT po.decision,pj.id FROM sub2api_enhance.third_party_prompt_audit_outcomes po JOIN sub2api_enhance.third_party_prompt_audit_jobs pj ON pj.id=po.job_id WHERE COALESCE(pj.source_job_id,pj.id)=j.source_job_id AND po.created_at<o.created_at ORDER BY pj.id DESC LIMIT 1) prior ON true WHERE j.run_kind='reaudit' AND prior.id<j.id AND prior.decision<>o.decision AND o.created_at>=$1 AND o.created_at<$2 AND ($3='' OR j.execution_mode=$3)) x`, args, &result.Reaudit},
		{`SELECT COALESCE(json_object_agg(failure_stage,n),'{}') FROM (SELECT COALESCE(failure_stage,'unknown') failure_stage,count(*) n FROM sub2api_enhance.third_party_prompt_audit_jobs WHERE run_kind='request' AND status='failed' AND finished_at>=$1 AND finished_at<$2 AND ($3='' OR execution_mode=$3) GROUP BY failure_stage) x`, args, &result.Failures},
		{`SELECT COALESCE(json_object_agg(decision,n),'{}') FROM (SELECT o.decision,count(*) n FROM sub2api_enhance.third_party_prompt_audit_events e JOIN sub2api_enhance.third_party_prompt_audit_outcomes o ON o.id=e.latest_outcome_id JOIN sub2api_enhance.third_party_prompt_audit_jobs j ON j.id=e.job_id WHERE e.created_at>=$1 AND e.created_at<$2 AND ($3='' OR j.execution_mode=$3) GROUP BY o.decision) x`, args, &result.Events},
		{`SELECT COALESCE(json_object_agg(gateway_result,n),'{}') FROM (SELECT gateway_result,count(*) n FROM sub2api_enhance.third_party_prompt_audit_jobs WHERE run_kind='request' AND execution_mode='blocking' AND created_at>=$1 AND created_at<$2 AND ($3='' OR execution_mode=$3) GROUP BY gateway_result) x`, args, &result.Gateway},
		{`SELECT row_to_json(x) FROM (SELECT count(gateway_duration_ms) count,percentile_cont(0.5) WITHIN GROUP(ORDER BY gateway_duration_ms) p50_ms,percentile_cont(0.95) WITHIN GROUP(ORDER BY gateway_duration_ms) p95_ms FROM sub2api_enhance.third_party_prompt_audit_jobs WHERE run_kind='request' AND execution_mode='blocking' AND created_at>=$1 AND created_at<$2 AND ($3='' OR execution_mode=$3)) x`, args, &result.GatewayLatency},
		{`SELECT row_to_json(x) FROM (SELECT count(*) count,percentile_cont(0.5) WITHIN GROUP(ORDER BY extract(epoch FROM finished_at-created_at)*1000) p50_ms,percentile_cont(0.95) WITHIN GROUP(ORDER BY extract(epoch FROM finished_at-created_at)*1000) p95_ms FROM sub2api_enhance.third_party_prompt_audit_jobs WHERE finished_at>=$1 AND finished_at<$2 AND ($3='' OR execution_mode=$3)) x`, args, &result.TaskLatency},
		{`SELECT row_to_json(x) FROM (SELECT count(*) FILTER(WHERE (reuse_metrics::json->>'whole_lookups')::bigint>0) whole_lookups,count(*) FILTER(WHERE (reuse_metrics::json->>'whole_hits')::bigint>0) whole_hits,COALESCE(sum((reuse_metrics::json->>'segment_lookups')::bigint),0) segment_lookups,COALESCE(sum((reuse_metrics::json->>'segment_hits')::bigint),0) segment_hits,COALESCE(sum((reuse_metrics::json->>'within_job_hits')::bigint),0) within_job_hits FROM sub2api_enhance.third_party_prompt_audit_jobs WHERE created_at>=$1 AND created_at<$2 AND ($3='' OR execution_mode=$3)) x`, args, &result.Reuse},
	...
	if err = tx.QueryRowContext(ctx, `SELECT count(*) FILTER(WHERE run_kind='request'),count(*) FILTER(WHERE run_kind='reaudit'),COALESCE(sum(attempts),0) FROM sub2api_enhance.third_party_prompt_audit_jobs WHERE created_at>=$1 AND created_at<$2 AND ($3='' OR execution_mode=$3)`, args...).Scan(&result.Received, &result.ReauditsCreated, &result.EvaluationRounds); err != nil {
		return nil, err
	}
