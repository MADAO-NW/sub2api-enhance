...
func (r *Repository) Stats(ctx context.Context, q StatsQuery) (*Stats, error) {
	...
	queries := []struct {
		...
		{`WITH latest AS (SELECT DISTINCT ON (o.job_id) o.id,j.created_at,j.execution_mode FROM sub2api_enhance.third_party_prompt_audit_outcomes o JOIN sub2api_enhance.third_party_prompt_audit_jobs j ON j.id=o.job_id ORDER BY o.job_id,o.audit_round DESC,o.id DESC), counts AS (SELECT count(*) FILTER(WHERE s.reuse_kind IN ('history','within_job','inflight','full_evaluation')) reused,count(*) total FROM latest JOIN sub2api_enhance.third_party_prompt_audit_outcome_segments s ON s.outcome_id=latest.id WHERE latest.created_at>=$1 AND latest.created_at<$2 AND ($3='' OR latest.execution_mode=$3)) SELECT json_build_object('reused',reused,'total',total,'rate',CASE WHEN total=0 THEN NULL ELSE reused::double precision/total END) FROM counts`, args, &result.SegmentReuse},
