...
默认联合审核触发阈值为 **50%**，违规阈值为 **80%**。提醒、累计停用默认关闭；启用时默认最近 10 次正式审核中 3 次违规提醒、累计 5 次违规自动停用。全局处理方式、阈值、节点聚合、提醒和停用规则作为默认值；新增“用户专属规则”时复制当前全局值，若全局处理方式为关闭则用户默认异步，之后独立维护，删除专属规则才恢复继承。全局关闭始终是总开关；用户可在全局启用时单独选择异步审核或同步阻断。现有“不审核用户”表示只保存原文并转发，不创建审核任务、模型调用或处置。窗口提醒适用于所有被审核用户，累计违规、自动停用和累计重置只适用于普通用户，因为原版 Sub2API 拒绝停用管理员。同一普通用户同时达到提醒和停用条件时会分别产生并投递两类动作。账号恢复入口位于配置页“用户违规累计管理”。
...
任务的同步或异步语义在请求入口确定，后续配置变化不会追溯改变已经转发或正在等待的请求。每次实际评估在首个模型调用前重新绑定该用户的最新审核配置，并在同一读锁内复制全部启用节点凭据；本轮随后使用固定的节点、顺序、地址、模型、参数、超时和内存凭据。保存新节点配置不会影响已经开始的评估，排队或重试任务在下一次评估时使用最新配置。全局切换为关闭时，已经发出的 HTTP 调用完成，尚未发送的节点停止；已有结果检查点只继续持久化，不重新审核。节点凭据不会写入任务、日志或调用元数据。
