...
默认联合审核触发阈值为 **50%**，违规阈值为 **80%**。提醒、累计停用默认关闭；启用时默认最近 10 次正式审核中 3 次违规提醒、累计 5 次违规自动停用。窗口提醒、累计违规、自动停用和累计重置只适用于普通用户；管理员与超管仍接受审核、同步阻断及每次阻断通知。同一普通用户同时达到提醒和停用条件时会分别产生并投递两类动作。账号恢复入口位于配置页“用户违规累计管理”。

上游拒绝、空 `choices`、超时等技术失败不会自动当作用户违规。空响应、缺失 `choices` 或消息内容等协议失败会在当前节点执行一次格式修正，仍失败后记录原始响应并继续后续启用节点；只有后续取得足够违规票才判定违规。重新审核在原任务上增加审核轮次，强制重新调用模型并更新事件的最新结论，任务 ID 与任务总数不变；事件和任务列表显示当前或最新成功轮次的审核耗时。不审核用户仍保存原文；已有任务的采集详情显示“查看任务”，无任务且可恢复时才显示恢复入口。

内置默认审核政策来自项目技术文档中的完整 R01—R14 正文，发送模型时不包含 Markdown 代码围栏。新配置和“恢复默认政策”使用当前版本；只有保存值与旧内置默认逐字一致时才会自动升级并增加配置版本，自定义提示词不会被覆盖。

HTTP 客户端可为同一会话的请求设置 `X-Enhance-Conversation-ID`。增强代理保存该标识用于会话内结果复用，并在转发前移除；WebSocket 使用连接标识。未提供可靠会话标识的 HTTP 请求各自审核，不按用户或正文猜测会话。不同会话不会相互复用或按用户串行；同会话完全相同的并发审核可合并一次模型评估，但每个业务请求仍分别保存任务、事件和处置事实。同步阻塞每次都会通知被阻塞用户；配置管理员邮箱时另行通知管理员，同地址自动去重。
