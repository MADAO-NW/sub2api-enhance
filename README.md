...
| 原服务 | 已运行的 Sub2API、PostgreSQL 15+、Redis 7+；额度结转和缓存修复还需访问原版实际配额键所在逻辑库 |
...
| `ENHANCE_REDIS_URL` | 审计统计投影和审核热缓存使用的 Redis 连接；必须使用独立 ACL、逻辑库及 `sub2api_enhance:` 键前缀权限 |
| `ENHANCE_AUDIT_CACHE_TTL` | 审核目标和完整结果热缓存时长，使用 Go duration（如 `168h`），必须显式填写 |
...
2. 填写审核节点地址和供应商 Key，点击“获取模型列表”并选择模型；列表不可用时可手动输入模型名，节点名称自动生成。接口地址可填写服务根地址、尾部 `/`、尾部 `/v1` 或带前缀的 `/openai/v1`，增强服务会统一拼接模型列表和 Chat Completions 路径。已保存节点的 Key 留空会保留原值。
3. 需要关闭思考或传递供应商专有参数时，在“审核节点高级配置（providers.json）”中编辑 `parameters`，应用到表单后再保存。凭据不会显示或保存到这段 JSON，`model`、`messages`、`stream` 由增强服务固定管理。
4. 使用“测试此节点”确认节点能返回合法评分。“查看调用详情”可查看上游原始响应；节点测试不会生成正式审核任务，也不会执行用户处罚。
5. 配置平台/分组、不审核用户和运行模式后保存。用户风险审核固定只选择协议中最新一条 user 消息：完整原文仍保存，但更早的 user 不进入用户风险目标。新增的 Agent/model/tool 有效行为会按结构化增量单独审核，结论用于阻断或复核行为，不归责于用户。无需打开原版“风控中心”开关。
6. 用现有客户端向已分流的公网模型 API 发起一条普通请求，核对“原文采集”新增记录，以及匹配的任务和采集 ID。仅测试审核节点，不能验证真实请求是否经过增强代理。
...
默认待复核／指令关联触发阈值为 **50%**，违规阈值为 **80%**。提醒、累计停用默认关闭；启用时默认最近 10 次正式审核中 2 次违规提醒、累计 1 次违规自动停用。“审核关闭时仍采集原文”对新配置默认开启；已经明确保存的开关和用户专属规则不会被默认值覆盖。全局处理方式、阈值、节点聚合、提醒和停用规则作为默认值；新增“用户专属规则”时复制当前全局值，若全局处理方式为关闭则用户默认异步，之后独立维护，删除专属规则才恢复继承。全局关闭始终是模型调用总开关；关闭期间原文正常采集和转发，但不创建自动任务。现有“不审核用户”同样保存原文并转发，不自动创建审核任务、模型调用或处置；管理员可从采集记录创建无处罚资格的异步任务，全局关闭期间任务只排队。窗口提醒适用于所有被审核用户，累计违规、自动停用和累计重置只适用于普通用户，因为原版 Sub2API 拒绝停用管理员。同一普通用户同时达到提醒和停用条件时会分别产生并投递两类动作。账号恢复入口位于配置页“用户违规累计管理”。

上游拒绝、空 `choices`、超时等技术失败不会自动当作用户违规。空响应、缺失 `choices` 或消息内容等协议失败会在当前节点执行一次格式修正，仍失败后记录原始响应并继续后续启用节点；只有后续取得足够违规票才判定违规。输入结构提取失败发生在审核模型调用之前，不会产生模型 Attempt；Responses 的 `additional_tools`、`compaction`、`encrypted_content`、agent/reasoning 和工具内容仍在原文及角色清单中保留。每个节点先审核最新 user 消息；新增的 Agent/model/tool 内容只有在形成有效行为候选时才进入 `effective_behavior` 阶段，普通分析和重复历史不会单独调用模型。有效行为结论可阻断当前转发，但不直接处罚用户。若 user 未直接违规，再检查可见 system/developer 指令基线；指令基线本身不能处罚用户。找不到 user 文本时仍不创建用户审核任务、不调用用户风险模型，并继续转发。

重新审核在原任务上增加审核轮次，默认允许复用全库精确匹配的审核目标或完整结果，也可选择强制重新调用模型；强制结果追加为缓存最新版本，不删除历史。缓存最小单位分别是完整的最新 user 消息、指令上下文、有效行为候选和两者关联，不能用旧单片段分数拼装新结论。最新 user 或有效行为片段的任一内容、块或顺序变化都会形成新目标；目标成功即独立入库，即使整任务后来失败也可由其他用户复用。任务 ID 与任务总数不变，任务列表直接展示最新成功结论、有效行为阻断状态、最新 user 文本片段数、目标复用率和审核耗时。不审核用户仍保存原文；已有任务的采集详情显示“查看任务”，无任务且可恢复时才显示恢复入口。

全部任务和原文采集列表均在同一身份项展示用户名、用户 ID 和邮箱；历史快照缺少用户名时仅为列表展示读取当前用户名，不改写原始快照。运行概览中的“代理转发状态”只描述增强代理从保存原文到转发原版的可观测进度：`not_forwarded` 尚未开始，`started` 已发往原版，`response_started` 已收到 HTTP 响应头，`complete` 表示 HTTP 响应体读完或 WebSocket 客户端消息已发送，`blocked` 表示增强入口未转发，`unknown` 表示无法确认最终阶段。它不证明模型供应商调用、计费或业务成功；具体时间和原因保留在采集详情的观测记录中。

任务的同步或异步语义在请求入口确定，后续配置变化不会追溯改变已经转发或正在等待的请求。每次实际评估在首个模型调用前重新绑定该用户的最新审核配置，并在同一读锁内复制全部启用节点凭据；本轮随后使用固定的节点集合、地址、模型、参数、超时和内存凭据。调度器每次从本次执行尚未使用的节点中按健康状态、活跃并发占比、候选等待数、延迟 EWMA 和轮询次序选择；每个节点默认最多并发 4 个任务，Worker 仍限制同时处理的任务总数。连续两次瞬时失败会熔断，429 优先服从 `Retry-After`，认证及固定配置错误需修改配置或管理员测试后恢复；异常节点冷却结束后才执行一次低优先级健康探测。健康探测是真实模型调用并保留 Attempt，但不创建 Job、Outcome、缓存或处置。

异步任务在首个模型请求尚未发出时遇到容量满载或临时冷却，会释放 Worker 并退回数据库队列，不增加任务执行次数；同步任务最多等待 5 秒，永久无健康节点时立即返回审核不可用。保存新节点配置不会影响已经开始的审核语义；节点并发缩容不取消已开始调用。排队或重试任务在下一次评估时使用最新配置。全局切换为关闭时，已经发出的 HTTP 调用完成，尚未发送的节点和健康探测停止；已有结果检查点只继续持久化，不重新审核。节点凭据不会写入任务、日志或调用元数据。用户被加入“不审核用户”后，尚未发出首个模型调用的排队、重试或已领取任务会直接标记为已跳过，不消耗审核轮次；首个调用已经发出的本轮继续保存审核证据，但不再触发提醒、累计违规或停用。再次移出不审核名单只影响新流量，历史已跳过任务和排除期间的待审核采集不会自动追补，管理员仍可显式发起无处罚人工审核。

嵌入 Sub2API 的页面会根据 `ui_mode=embedded` 使用 iframe 全部可用宽度；独立窗口继续限制阅读宽度。任务的额外查询条件只能点击文字按钮展开，宽屏表格使用更多可用空间，中小屏保持横向滚动。

运行概览按当前时间与处理方式筛选，使用每个任务最新成功 Outcome 计算全局和逐用户审核目标复用率；两者均显示“百分比　复用数 / 总目标数”，逐用户身份优先读取原版当前用户名和邮箱。“模型实际调用”按节点、来源和阶段统计，因此同一节点可能分别出现 current user、指令上下文、关联裁决、格式修正、管理员测试和健康探测，记录数不等于配置节点数。概览优先读取与 PostgreSQL 当前事实水位一致的 Redis 统计投影；首次查询从数据库建立投影，已有旧投影时先返回旧快照并在后台刷新，页面显示统计来源、数据时点和回建状态。Redis 只保存可重建投影及带有效期的审核热缓存，Capture、Job、Outcome、Attempt 和处罚事实仍以 PostgreSQL 为准。

内置默认审核政策来自项目技术文档中的完整 R01—R14 正文，发送模型时不包含 Markdown 代码围栏。新配置和“恢复默认政策”使用当前版本；只有保存值与旧内置默认逐字一致时才会自动升级并增加配置版本，自定义提示词不会被覆盖。

HTTP 客户端可为请求设置 `X-Enhance-Conversation-ID`，增强代理保存后在转发前移除；WebSocket 使用连接标识。会话标识只辅助协议提取器还原当前任务范围，不作为缓存隔离条件。节点语义、政策、固定协议、目标类型和完整有序内容一致的审核目标可跨用户、跨会话复用，完整目标与评估指纹一致时也可复用完整结果；同实例并发的相同目标或完整任务只由一个领头请求调用模型。每个业务请求仍分别保存任务、网关和处置事实。同步阻塞每次都会通知被阻塞用户；配置管理员邮箱时另行通知管理员，同地址自动去重。
...
5. 需要管理员立即处理时，点击“立即重置”并确认；系统会按当前目标分组和已勾选的日/周窗口，逐个调用原版 Sub2API 重置接口处理全部参加用户。该操作不等待自动检测边界，也不受观察模式限制；部分用户失败时页面会保留逐项结果。
...
原版自然日和周一规则继续保留；周一结转保留或加回旧周已用额度，再等待 OpenAI 账号窗口确认推进后归零。快照过旧、窗口冲突或错过安全窗口会跳过，不追补历史。普通额度归零调用原版管理 API，因此在原版“操作日志”留下 `audit_logs` 证据，但它不是模型调用，不会出现在原版模型“使用记录”中；周一结转直接在增强事务内维护配额，也不会生成原版操作日志。DB、Redis 与原版并发计费不能保证跨系统强一致；结果不确定时使用记录详情中的只读核对，不盲目重复归零。

## 更新与日常维护

升级到包含 Redis 审计投影的版本前，必须先在现有环境文件中补齐 `ENHANCE_REDIS_URL` 和 `ENHANCE_AUDIT_CACHE_TTL`，并确认独立 ACL/逻辑库可以连接；缺少任一配置或 Redis 不可达时新进程会拒绝启动。首次启动还会执行 `007`，为审计 Attempt 回填投影水位并建立增量索引，应先完成数据库备份并预留迁移负载窗口。
...
已适配 Responses、Chat Completions、Messages、Gemini 等文本请求及 Responses WebSocket，并支持 identity、gzip、deflate、zstd 请求正文。仅审核客户端实际携带的输入，不主动获取上游隐含历史，不审核模型新生成的输出。multipart 保存文本字段及二进制描述；未知协议或不支持的编码不会被宣称为已完成审核。原文采集页可预览并恢复全部自动来源失败记录：结果检查点只恢复落库，已有任务原地增加轮次，无任务的失败采集创建异步任务，均不会重放原业务请求。未适配的 WebSocket（包括部分 Realtime）在启用审计时明确报错。

全部任务支持按实际审核模型名称模糊查询；原文采集支持时间、用户/API Key/分组、关键词、协议、完整性、身份资格、处理状态和转发状态筛选。两个列表均可选择每页 20、50、100、200 条，并提供首页、上一页、指定页、下一页和末页跳转。
