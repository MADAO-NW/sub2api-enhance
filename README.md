...
5. 配置平台/分组、不审核用户和运行模式后保存。用户风险审核固定只选择协议中最新一条 user 消息：完整原文仍保存，但更早的 user、assistant/model、tool 或 reasoning 不进入用户风险目标。无需打开原版“风控中心”开关。
...
上游拒绝、空 `choices`、超时等技术失败不会自动当作用户违规。空响应、缺失 `choices` 或消息内容等协议失败会在当前节点执行一次格式修正，仍失败后记录原始响应并继续后续启用节点；只有后续取得足够违规票才判定违规。输入结构提取失败发生在审核模型调用之前，不会产生模型 Attempt；Responses 的 `additional_tools`、`compaction`、`encrypted_content`、agent/reasoning 和工具内容仍在原文及角色清单中保留，但不进入自动风险目标。每个节点先审核最新 user 消息，同一消息的多个文本块按原顺序保留；若 user 未直接违规，再检查可见 system/developer 指令基线，只有指令风险达到触发阈值才追加一次 user 与指令的关联裁决。指令基线本身不能处罚用户。找不到 user 文本时不创建自动任务、不调用模型，并继续转发。

重新审核在原任务上增加审核轮次，默认允许复用全库精确匹配的审核目标或完整结果，也可选择强制重新调用模型；强制结果追加为缓存最新版本，不删除历史。缓存最小单位分别是完整的最新 user 消息、指令上下文和两者关联，不能用旧单片段分数拼装新结论。最新 user 的任一内容、块或顺序变化都会形成新目标；目标成功即独立入库，即使整任务后来失败也可由其他用户复用。任务 ID 与任务总数不变，任务列表直接展示最新成功结论、最新 user 文本片段数、目标复用率和审核耗时。不审核用户仍保存原文；已有任务的采集详情显示“查看任务”，无任务且可恢复时才显示恢复入口。
