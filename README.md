...
5. 配置平台/分组、不审核用户和运行模式后保存。审核范围固定为“当前任务 user 合并审核”：完整原文仍保存，但自动审核不再纳入历史 user、assistant/model、tool 或 reasoning。无需打开原版“风控中心”开关。
...
上游拒绝、空 `choices`、超时等技术失败不会自动当作用户违规。空响应、缺失 `choices` 或消息内容等协议失败会在当前节点执行一次格式修正，仍失败后记录原始响应并继续后续启用节点；只有后续取得足够违规票才判定违规。输入结构提取失败发生在审核模型调用之前，不会产生模型 Attempt；Responses 的 `additional_tools`、`compaction`、`encrypted_content`、agent/reasoning 和工具内容仍在原文及角色清单中保留，但不进入自动风险目标。每个节点先把全部“当前任务 user”按协议顺序合并为一个目标并只审核一次；若 user 未直接违规，再检查可见 system/developer 指令基线，只有指令风险达到触发阈值才追加一次 user 与指令的关联裁决。指令基线本身不能处罚用户。找不到当前任务 user 文本时不创建自动任务、不调用模型，并继续转发。

重新审核在原任务上增加审核轮次，默认允许复用全库精确匹配的审核目标或完整结果，也可选择强制重新调用模型；强制结果追加为缓存最新版本，不删除历史。缓存最小单位分别是完整的当前 user 合集、指令上下文和两者关联，不能用旧单片段分数拼装新结论。任一 user 内容、块或顺序变化都会形成新合集；目标成功即独立入库，即使整任务后来失败也可由其他用户复用。任务 ID 与任务总数不变，任务列表直接展示最新成功结论、当前 user 合并条数、目标复用率和审核耗时。不审核用户仍保存原文；已有任务的采集详情显示“查看任务”，无任务且可恢复时才显示恢复入口。
...
异步任务在首个模型请求尚未发出时遇到容量满载或临时冷却，会释放 Worker 并退回数据库队列，不增加任务执行次数；同步任务最多等待 5 秒，永久无健康节点时立即返回审核不可用。保存新节点配置不会影响已经开始的审核语义；节点并发缩容不取消已开始调用。排队或重试任务在下一次评估时使用最新配置。全局切换为关闭时，已经发出的 HTTP 调用完成，尚未发送的节点和健康探测停止；已有结果检查点只继续持久化，不重新审核。节点凭据不会写入任务、日志或调用元数据。

嵌入 Sub2API 的页面会根据 `ui_mode=embedded` 使用 iframe 全部可用宽度；独立窗口继续限制阅读宽度。任务的额外查询条件只能点击文字按钮展开，宽屏表格使用更多可用空间，中小屏保持横向滚动。

运行概览按当前时间与处理方式筛选，使用每个任务最新成功 Outcome 计算全局和逐用户审核目标复用率；两者均显示“百分比　复用数 / 总目标数”，逐用户身份优先读取原版当前用户名和邮箱。“模型实际调用”按节点、来源和阶段统计，因此同一节点可能分别出现 current user、指令上下文、关联裁决、格式修正、管理员测试和健康探测，记录数不等于配置节点数。
