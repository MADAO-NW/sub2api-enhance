...
默认联合审核触发阈值为 **50%**，违规阈值为 **80%**。提醒、累计停用默认关闭；启用时默认最近 10 次正式审核中 3 次违规提醒、累计 5 次违规自动停用。全局处理方式、阈值、节点聚合、提醒和停用规则作为默认值；新增“用户专属规则”时复制当前全局值，若全局处理方式为关闭则用户默认异步，之后独立维护，删除专属规则才恢复继承。全局关闭始终是模型调用总开关；可另行开启“审核关闭时仍采集原文”，此时请求正常转发、采集进入待管理员审核，采集失败不阻断业务。现有“不审核用户”同样保存原文并转发，不自动创建审核任务、模型调用或处置；管理员可从采集记录创建无处罚资格的异步任务，全局关闭期间任务只排队。窗口提醒适用于所有被审核用户，累计违规、自动停用和累计重置只适用于普通用户，因为原版 Sub2API 拒绝停用管理员。同一普通用户同时达到提醒和停用条件时会分别产生并投递两类动作。账号恢复入口位于配置页“用户违规累计管理”。

上游拒绝、空 `choices`、超时等技术失败不会自动当作用户违规。空响应、缺失 `choices` 或消息内容等协议失败会在当前节点执行一次格式修正，仍失败后记录原始响应并继续后续启用节点；只有后续取得足够违规票才判定违规。输入结构提取失败发生在审核模型调用之前，不会产生模型 Attempt；Responses 的 `additional_tools` 项作为工具上下文保留，不再因缺少 `content` 而报 `input_parse_failed`。重新审核在原任务上增加审核轮次，默认允许复用全库精确匹配的片段或完整结果，也可选择强制重新调用模型；任务 ID 与任务总数不变，任务列表直接展示最新成功结论和审核耗时。不审核用户仍保存原文；已有任务的采集详情显示“查看任务”，无任务且可恢复时才显示恢复入口。
