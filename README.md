...
5. 选择审核范围、平台/分组和不审核用户，设置运行模式后保存。新配置默认审核当前用户轮次与生效指令；已有配置保持原范围。无需打开原版“风控中心”开关。
...
任务的同步或异步语义在请求入口确定，后续配置变化不会追溯改变已经转发或正在等待的请求。每次实际评估在首个模型调用前重新绑定该用户的最新审核配置，并在同一读锁内复制全部启用节点凭据；本轮随后使用固定的节点集合、地址、模型、参数、超时和内存凭据。节点不再按配置位置固定串行，而是每次从本次执行尚未使用的节点中选择当前负载最低者；每个节点默认最多并发 4 个任务，Worker 仍限制同时执行的任务总数。保存新节点配置不会影响已经开始的审核语义；节点并发缩容不取消已开始调用。排队或重试任务在下一次评估时使用最新配置。全局切换为关闭时，已经发出的 HTTP 调用完成，尚未发送的节点停止；已有结果检查点只继续持久化，不重新审核。节点凭据不会写入任务、日志或调用元数据。

嵌入 Sub2API 的页面会根据 `ui_mode=embedded` 使用 iframe 全部可用宽度；独立窗口继续限制阅读宽度。任务的额外查询条件只能点击文字按钮展开，宽屏表格使用更多可用空间，中小屏保持横向滚动。

运行概览按当前时间与处理方式筛选，使用每个任务最新成功 Outcome 计算全局和逐用户片段复用率；两者均显示“百分比　复用数 / 总片段数”，逐用户身份优先读取原版当前用户名和邮箱。

内置默认审核政策来自项目技术文档中的完整 R01—R14 正文，发送模型时不包含 Markdown 代码围栏。新配置和“恢复默认政策”使用当前版本；只有保存值与旧内置默认逐字一致时才会自动升级并增加配置版本，自定义提示词不会被覆盖。

HTTP 客户端可为请求设置 `X-Enhance-Conversation-ID`，增强代理保存后在转发前移除；WebSocket 使用连接标识。会话标识用于还原输入轮次，不再限制缓存：节点语义、政策、固定协议、角色、轮次范围和文本完全一致的片段可跨用户、跨会话复用，完整目标与评估指纹一致时也可复用完整结果。同实例并发的相同片段或完整任务只由一个领头请求调用模型；每个业务请求仍分别保存任务、网关和处置事实。同步阻塞每次都会通知被阻塞用户；配置管理员邮箱时另行通知管理员，同地址自动去重。
