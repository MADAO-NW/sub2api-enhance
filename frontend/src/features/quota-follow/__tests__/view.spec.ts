...
const mocks=vi.hoisted(()=>({config:vi.fn(),save:vi.fn(),runtime:vi.fn(),discovery:vi.fn(),records:vi.fn(),detail:vi.fn(),reconcile:vi.fn(),repairCache:vi.fn(),events:vi.fn(),event:vi.fn(),groups:vi.fn(),showError:vi.fn(),showSuccess:vi.fn()}))
...
beforeEach(()=>{
 ...
 mocks.discovery.mockResolvedValue({accounts:[],users:[]});mocks.records.mockResolvedValue({items:[],total:0});mocks.events.mockResolvedValue({items:[],total:0});mocks.save.mockResolvedValue({revision:4})
})
