  ...
  <div class="audit-records space-y-5">
    <form class="card space-y-4 p-5" @submit.prevent="apply">
      ...
      <div><button type="button" class="inline-flex w-fit items-center gap-1 text-sm" :aria-expanded="advancedOpen" data-test="advanced-filter-toggle" @click="advancedOpen = !advancedOpen"><span aria-hidden="true">{{ advancedOpen ? '▾' : '▸' }}</span><span>{{ label('from') }} / {{ label('userID') }} / {{ label('modelID') }}</span></button><div v-show="advancedOpen" class="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-4" data-test="advanced-filters">
        ...
        <label class="space-y-1 text-sm"><span>{{ label('modelID') }}</span><input v-model="filters.model_id" class="input" /></label><label class="space-y-1 text-sm"><span>{{ label('auditModel') }}</span><input v-model="filters.model_name" class="input" /></label><label class="space-y-1 text-sm"><span>{{ label('platform') }}</span><input v-model="filters.platform" class="input" /></label>
    ...
    <div class="card overflow-hidden"><div class="overflow-x-auto"><table class="w-full text-left text-sm"><thead class="bg-gray-50 text-gray-500 dark:bg-dark-900 dark:text-dark-400"><tr><th class="p-3"><input type="checkbox" :aria-label="label('all')" :checked="rows.length > 0 && selected.length === rows.length" @change="selected = ($event.target as HTMLInputElement).checked ? rows.map(row => row.id) : []" /></th><th class="p-3">{{ label('created') }}</th><th class="p-3">{{ label('captureID') }}</th><th class="p-3">{{ label('identity') }}</th><th class="p-3">{{ label('platform') }} / {{ label('model') }}</th><th class="p-3">{{ label('status') }} / {{ label('decision') }}</th><th class="p-3">{{ label('auditDuration') }}</th><th class="p-3">{{ label('inputStatus') }}</th><th class="p-3">{{ label('detail') }}</th></tr></thead><tbody>
      <tr v-for="row in rows" :key="row.id" class="border-t border-gray-100 align-top dark:border-dark-700">
        <td class="p-3"><input v-model="selected" type="checkbox" :value="row.id" :aria-label="`#${row.id}`" /></td>
        <td class="whitespace-nowrap p-3"><p>{{ formatTime(row.created_at) }}</p><p class="mt-1 text-xs text-gray-500">#{{ row.id }} · {{ label(row.current_run_kind || 'request') }} · {{ label('auditRound') }} {{ row.outcome?.audit_round ?? row.audit_round }}</p><p class="mt-1 max-w-64 whitespace-normal break-all text-xs text-gray-500">{{ label('requestID') }}: {{ row.request_id || '—' }}</p></td>
        <td class="p-3"><button v-if="row.capture_id" class="text-primary-600 underline" :disabled="captureLoadingID === row.capture_id" @click="showCapture(row.capture_id)">#{{ row.capture_id }}</button><span v-else>—</span></td>
        <td class="min-w-48 max-w-80 break-words p-3"><p>(#{{ row.user_id }})<span v-if="row.display_username || row.identity.username" class="ml-1">{{ row.display_username || row.identity.username }}</span></p><p>{{ row.display_email || row.identity.user_email }}</p><p class="text-xs text-gray-500">{{ row.identity.api_key_name }} · {{ row.identity.group_name }}</p></td>
        <td class="max-w-72 break-words p-3"><p>{{ row.platform }} · {{ row.requested_model }}</p><p class="mt-1 text-xs text-gray-500">{{ row.protocol }} · {{ label(row.execution_mode) }}</p><p class="mt-1 text-xs">{{ label(row.gateway_result) }}</p></td>
        <td class="min-w-40 p-3">
          <p>{{ label(row.status) }}</p><strong v-if="row.outcome" :class="row.outcome.decision === 'block' ? 'text-red-600 dark:text-red-400' : row.outcome.decision === 'review' ? 'text-amber-700 dark:text-amber-400' : ''">{{ label(row.outcome.decision) }}</strong>
          <p v-if="row.original_decision && row.original_decision !== row.outcome?.decision" class="text-xs text-gray-500">{{ label('originalDecision') }} {{ label(row.original_decision) }} → {{ label('latestDecision') }} {{ label(row.outcome?.decision ?? row.status) }}</p>
          <p v-if="row.outcome?.partial_failure" class="text-xs text-amber-700">{{ label('partial_failure') }}</p>
          <div v-for="model in row.outcome?.models ?? []" :key="model.model_id" class="mt-2"><strong class="text-xs">{{ model.model_name }}</strong><NodeDecision :model="model" :config="row.outcome?.decision_config" /></div>
          <p v-if="row.current_user_count" class="mt-1 text-xs">{{ label('latestUserFragments') }}: {{ row.current_user_count }} {{ label('userItemsUnit') }}</p>
          <p v-if="row.outcome" class="mt-1 text-xs">{{ label('segmentReuseRate') }} <template v-if="row.outcome.segment_reuse?.rate != null">{{ Math.round(row.outcome.segment_reuse.rate * 100) }}% <span class="ml-2">{{ row.outcome.segment_reuse.reused }} / {{ row.outcome.segment_reuse.total }}</span></template><template v-else>—</template></p>
		  <p class="mt-1 text-xs"><LatestUserContent source="job" :id="row.id" variant="popover" /></p><p class="mt-1 text-xs">{{ label('attempts') }} {{ row.attempts }} / {{ row.max_attempts }}</p><p v-if="row.last_error_code" class="mt-1 break-all text-xs text-red-600 dark:text-red-400"><span v-if="row.failure_stage">{{ label(row.failure_stage) }} · </span>{{ row.last_error_code }}<span v-if="row.last_error_message"> · {{ row.last_error_message }}</span></p><template v-if="row.status === 'retry'"><p class="mt-1 text-xs">{{ label('nextRetry') }} {{ formatTime(row.next_attempt_at) }}</p><p class="mt-1 text-xs text-gray-500 dark:text-dark-400">{{ label('retryHint') }}</p></template>
        </td>
        <td class="whitespace-nowrap p-3">{{ formatMS(row.outcome?.duration_ms ?? row.duration_ms) }}</td><td class="p-3 text-xs">{{ label(row.snapshot_status) }}</td><td class="p-3"><button class="btn btn-secondary btn-sm whitespace-nowrap" @click="detailID = row.id">{{ label('detail') }}</button></td>
      </tr>
      <tr v-if="!loading && !rows.length"><td colspan="9" class="p-10 text-center text-gray-500">{{ label('empty') }}</td></tr>
    </tbody></table></div><Pagination :page="page" :page-size="pageSize" :page-size-options="[20, 50, 100, 200]" :total="total" @update:page="changePage" @update:page-size="changeSize" /></div>
