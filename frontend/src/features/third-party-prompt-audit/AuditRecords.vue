...
import { thirdPartyPromptAuditAPI as api, type AuditCapture, type AuditEvent, type AuditFilter, type AuditJob, type ReauditRequest, type ReauditResult } from '@/api/admin/third-party-prompt-audit'
...
const rows = ref<{ id: number; job: AuditJob; event?: AuditEvent }[]>([])
...
async function load() {
  ...
  try {
    if (props.source === 'events') {
      const result = await api.events(applied.value, page.value, pageSize.value)
      if (current === generation) { rows.value = result.items.flatMap(event => event.job ? [{ id: event.id, job: event.job, event }] : []); total.value = result.total }
    } else {
      const result = await api.jobs(applied.value, page.value, pageSize.value)
      if (current === generation) { rows.value = result.items.map(job => ({ id: job.id, job })); total.value = result.total }
    }
...
async function openPreview(useSelection: boolean) {
  previewRequest.value = { source: props.source, filter: JSON.parse(JSON.stringify({ ...applied.value, ...(useSelection ? { ids: selected.value } : {}) })) as AuditFilter }
  preview.value = null
...
async function submit() {
  ...
  previewLoading.value = true
  try {
    preview.value = await api.reaudit(previewRequest.value)
...
<template>
  <div class="space-y-5">
    <form class="card space-y-4 p-5" @submit.prevent="apply">
      ...
      <details><summary class="cursor-pointer text-sm">{{ label('from') }} / {{ label('userID') }} / {{ label('modelID') }}</summary><div class="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <label v-if="source === 'jobs'" class="space-y-1 text-sm"><span>{{ label('source') }}</span><select v-model="filters.run_kind" class="input"><option value="">{{ label('all') }}</option><option value="request">{{ label('request') }}</option><option value="reaudit">{{ label('reaudit') }}</option></select></label>
      </div></details>
    ...
    <div class="card overflow-hidden"><div class="overflow-x-auto"><table class="w-full text-left text-sm"><thead class="bg-gray-50 text-gray-500 dark:bg-dark-900 dark:text-dark-400"><tr><th class="p-3"><input type="checkbox" :aria-label="label('all')" :checked="rows.length > 0 && selected.length === rows.length" @change="selected = ($event.target as HTMLInputElement).checked ? rows.map(row => row.id) : []" /></th><th class="p-3">{{ label('created') }}</th><th class="p-3">{{ label('captureID') }}</th><th class="p-3">{{ label('identity') }}</th><th class="p-3">{{ label('platform') }} / {{ label('model') }}</th><th class="p-3">{{ label(source === 'events' ? 'decision' : 'status') }}</th><th class="p-3">{{ label('auditDuration') }}</th><th class="p-3">{{ label('inputStatus') }}</th><th class="p-3">{{ label('detail') }}</th></tr></thead><tbody>
      <tr v-for="row in rows" :key="row.id" class="border-t border-gray-100 align-top dark:border-dark-700"><td class="p-3"><input v-model="selected" type="checkbox" :value="row.id" :aria-label="`#${row.id}`" /></td><td class="whitespace-nowrap p-3"><p>{{ formatTime(row.event?.created_at ?? row.job.created_at) }}</p><p class="mt-1 text-xs text-gray-500">#{{ row.id }} · {{ label(row.job.current_run_kind || row.job.run_kind) }} · {{ label('auditRound') }} {{ row.event?.latest?.audit_round ?? row.job.audit_round }}</p></td><td class="p-3"><button v-if="row.job.capture_id" class="text-primary-600 underline" :disabled="captureLoadingID === row.job.capture_id" @click="showCapture(row.job.capture_id)">#{{ row.job.capture_id }}</button><span v-else>—</span></td><td class="min-w-48 max-w-80 break-words p-3"><p>{{ row.job.identity.username }} (#{{ row.job.user_id }})</p><p>{{ row.job.identity.user_email }}</p><p class="text-xs text-gray-500">{{ row.job.identity.api_key_name }} · {{ row.job.identity.group_name }}</p></td><td class="max-w-72 break-words p-3"><p>{{ row.job.platform }} · {{ row.job.requested_model }}</p><p class="mt-1 text-xs text-gray-500">{{ row.job.protocol }} · {{ label(row.job.execution_mode) }}</p><p class="mt-1 text-xs">{{ label(row.job.gateway_result) }}</p></td><td class="min-w-40 p-3"><strong :class="row.event?.latest?.decision === 'block' ? 'text-red-600 dark:text-red-400' : row.event?.latest?.decision === 'review' ? 'text-amber-700 dark:text-amber-400' : ''">{{ label(row.event?.latest?.decision ?? row.job.status) }}</strong><p v-if="row.event?.latest?.partial_failure" class="text-xs text-amber-700">{{ label('partial_failure') }}</p><div v-for="model in row.event?.latest?.models ?? []" :key="model.model_id" class="mt-2"><strong class="text-xs">{{ model.model_name }}</strong><NodeDecision :model="model" :config="row.event?.latest?.decision_config" /></div><p v-if="row.event?.reaudit_status" class="mt-1 text-xs">{{ label('reaudit') }}: {{ label(row.event.reaudit_status) }}</p><p class="mt-1 text-xs">{{ label('attempts') }} {{ row.job.attempts }} / {{ row.job.max_attempts }}</p><p v-if="row.job.last_error_code" class="mt-1 break-all text-xs text-red-600 dark:text-red-400">{{ label(row.job.failure_stage) }} · {{ row.job.last_error_code }}</p><p v-if="row.job.status === 'retry'" class="mt-1 text-xs">{{ label('nextRetry') }} {{ formatTime(row.job.next_attempt_at) }}</p></td><td class="whitespace-nowrap p-3">{{ formatMS(row.event?.latest?.duration_ms ?? row.job.duration_ms) }}</td><td class="p-3 text-xs">{{ label(row.job.snapshot_status) }}</td><td class="p-3"><button class="btn btn-secondary btn-sm whitespace-nowrap" @click="detailID = row.id">{{ label('detail') }}</button></td></tr>
