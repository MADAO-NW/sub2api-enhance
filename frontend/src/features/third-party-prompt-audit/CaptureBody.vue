...
<template>
  <div class="space-y-4">
    <dl class="grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div><dt class="text-gray-500 dark:text-dark-400">{{ label('eligibilityStatus') }}</dt><dd>{{ label(capture.eligibility_status || 'unknown') }}</dd></div>
      <div><dt class="text-gray-500 dark:text-dark-400">{{ label('derivedStatus') }}</dt><dd>{{ label(capture.processing_status || 'unknown') }}</dd></div>
    </dl>
    <p v-if="capture.identity?.reason" class="rounded-lg bg-amber-50 p-3 text-sm text-amber-800 dark:bg-amber-950/30 dark:text-amber-300">{{ label('eligibilityReason') }}: {{ capture.identity.reason }}</p>
