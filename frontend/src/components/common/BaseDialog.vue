...
const props=withDefaults(defineProps<{show:boolean;title:string;width?:string;closeOnEscape?:boolean;closeOnClickOutside?:boolean}>(),{closeOnEscape:true,closeOnClickOutside:false})
...
<template><Teleport to="body"><div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" @click.self="closeOnClickOutside&&emit('close')"><section ref="dialog" role="dialog" aria-modal="true" :aria-label="title" tabindex="-1" class="max-h-[90vh] w-full max-w-6xl overflow-auto rounded-2xl bg-white p-6 shadow-xl dark:bg-dark-900" @keydown="onKey"><header class="mb-5 flex items-center justify-between"><h2 class="text-xl font-semibold">{{ title }}</h2><button aria-label="关闭 / Close" @click="emit('close')">×</button></header><slot/><footer class="mt-5"><slot name="footer"/></footer></section></div></Teleport></template>
