...
<template>
  <main class="p-4 pt-3 sm:p-6 sm:pt-4">
    <div class="mx-auto max-w-[1600px] space-y-6 pb-8">
      ...
      <AuditRecords v-if="visited.includes('events')" v-show="active === 'events'" source="events" :refresh-key="refreshKey" @reaudit-created="trackReaudits" />
      <AuditRecords v-if="visited.includes('jobs')" v-show="active === 'jobs'" source="jobs" :initial-filter="jobFilter" :refresh-key="refreshKey" @reaudit-created="trackReaudits" />
